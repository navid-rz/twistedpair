...
from channel.simple import copper_channel
...
# --- Eye diagram calculation and plot (pre-channel) ---
sim_sps = int(sim.sps)
if sim_sps <= 0:
...
if eye_traces:
    ...
    plot_eye(eye_traces, sim_sample_rate=sample_rate, title=f"Eye Diagram (pre-channel, {segment_symbols}-symbol overlay)")
else:
    print("Not enough samples to build pre-channel eye diagram.")

# --- Apply copper channel and analyze ---
# Build a simple channel config matching channel.simple.copper_channel expectations
channel_cfg = SimpleNamespace(
    alpha_db_per_in_ghz=0.1,   # copper loss coeff
...
plot_waveform(waveform_chan[:5000], time[:5000], title="TX Waveform after Copper Channel")
